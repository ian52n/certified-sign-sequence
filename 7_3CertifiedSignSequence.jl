############################################################
# 7_3CertifiedSignSequence.jl — MULTI-SEQUENCE (v7.3)
#
# What this version does:
# • Picks a reproducible random initial point (dedicated RNG).
# • Does fast, *uncertified* burn-in with OrdinaryDiffEq to get
#   near the Lorenz attractor.
# • Runs CHUNKED, *rigorous* certified integration to produce a
#   sign sequence (L/R/?) and stops at the FIRST uncertifiable
#   crossing candidate, using local validated bracketing:
#     - Existence: endpoint sign change across z=SECTION
#     - Direction: inf(dz/dt) > 0 on hull of {lo, mid, hi}
#     - Label: uniform y-sign on accepted bracket (L/R/?)
# • If more sequences are requested, it uses the center of the
#   failing slab (or last box) as a point estimate, takes a
#   reproducible *uncertified* random hop, and repeats.
# • Writes/appends a .txt report in the requested format.
#   If the report already exists, it prepends:
#   "This file contains multiple runs"
#
# Reproducibility:
# • All randomness (initial point + hop durations) uses a
#   dedicated MersenneTwister seeded by --seed (default fixed).
#
# Notes:
# • This script relies on ReachabilityAnalysis (TMJets21a) for
#   validated flowpipes and OrdinaryDiffEq (Tsit5) for fast hops.
############################################################
...
# OrdinaryDiffEq version for uncertified steps (burn-in / hops)
...
Base.@kwdef mutable struct Config
    seed::Int                         = 930415324    # fixed default for reproducibility
    burn_in::Float64                  = 5.0
    num_sequences::Int                = 2
    ...
    # Local bracketing algorithm (rigorous)
...
function parse_kv!(cfg::Config, arg::AbstractString)
    ...
    elseif k == "--init-range"          cfg.init_range = parse_init_range(v)
    end
    return cfg
end

function get_config()::Config
    cfg = Config()  # fixed default seed -> reproducible by default
    for a in ARGS
        parse_kv!(cfg, a)
    end
...
# ============================
# Local validated bracketing (rigorous)
# ============================
...
# If report exists and doesn't start with the header line, prepend it.
function ensure_multi_runs_header!(path::String)
    header = "This file contains multiple runs"
    if isfile(path)
        content = read(path, String)
        firstline = isempty(content) ? "" : first(split(content, '\n'; limit=2))
        if firstline != header
            open(path, "w") do io
                println(io, header)
                println(io)
...
function append_run_summary!(
    ...
    existed = ensure_multi_runs_header!(cfg.output_path)
    open(cfg.output_path, existed ? "a" : "w") do io
        if existed
...
function main()
    ...
    # Burn-in (uncertified) to get near the attractor
    ...
    for seq_idx in 1:cfg.num_sequences
        ...
        # Next starting point: center of failing slab if any, else center of last box
